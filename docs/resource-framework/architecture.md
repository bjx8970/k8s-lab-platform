...
业务权限、关机投票、实验环境编排、地址分配继续位于应用其他模块。核心与基础插件不依赖 Flask、current_user、教师/学生/课程模型，也不包含可注入的业务审批或依赖校验钩子。
...
框架可以存储外部模块登记的逻辑资源并调用其 handler，但不解释该模块的组合语义。旧实验环境蓝图由外部编排模块自行管理，本框架仅接收展开后的具体请求；见[独立指令示例](examples/resource-commands.json)。
